...
namespace Net
{
    Listener::Listener( ProtocolID protocolId, float timeout = 10.0f ) {
    ...
    void Listener::Update( double deltaTime ) {
        ...
        while ( true ) {
            ...
            ProtocolID packet_protocolId;
            Port packet_ServerPort;
            ...
            Serialization::ReadShort( packet + 8, packet_ServerPort );
    ...
    ListenerEntry * Listener::FindEntry( const ListenerEntry & entry ) {
        ...
        {
            if (entries[i].address == entry.address &&
                strcmp(entries[i].name, entry.name) == 0)
            {
                return &entries[i];
            }
        }
