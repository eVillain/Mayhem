...
namespace Net
{
    Mesh::Mesh(ProtocolID protocolId,
               int maxNodes,
               float sendRate,
               float timeout)
    {
    ...
    bool Mesh::IsNodeConnected(NodeID nodeId)
    {
        assert(nodeId >= 0);
    ...
    Address Mesh::GetNodeAddress( NodeID nodeId )
    {
        assert( nodeId >= 0 );
        assert( nodeId < (int) nodes.size() );
    ...
    void Mesh::ProcessPacket(const Address& sender,
                             unsigned char data[],
                             int size)
    {
        ...
        uint32_t crcFromDataHeader = 0;
        Serialization::ReadInteger(data, crcFromDataHeader);
        Serialization::WriteInteger(data, protocolId);
        uint32_t crcPacket = 0;
        crcPacket = crc32c(crcPacket, data, size);
        if (crcFromDataHeader != crcPacket)
            return;
        ...
        while ( sendAccumulator > sendRate )
        {
            for ( unsigned int i = 0; i < nodes.size(); ++i )
            {
                if ( nodes[i].mode == NodeState::ConnectionAccept )
                {
                    ...
                    writeCRCWithProtocolID(packet, 7);
                ...
                else if ( nodes[i].mode == NodeState::Connected )
                {
                    ...
                    const int packetSize = (int)(5+10*nodes.size());
                    ...
                    writeCRCWithProtocolID(packet, packetSize);
    ...
    void Mesh::writeCRCWithProtocolID(unsigned char* data, const uint32_t size)
    {
        Serialization::WriteInteger(data, protocolId);
        uint32_t crc = 0;
