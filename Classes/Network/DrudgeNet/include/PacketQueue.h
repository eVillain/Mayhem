...
namespace Net
{
    struct PacketData
    {
        PacketSequenceID sequence;	    // packet sequence number
        float time;					    // time offset since packet was sent or received (depending on context)
        int size;						// packet size in bytes
    };
    
    inline bool IsSequenceMoreRecent(PacketSequenceID s1,
                                     PacketSequenceID s2,
    ...
    public:
        ...
        bool Exists(PacketSequenceID sequence)
        {
        ...
        void InsertSorted(const PacketData& p, PacketSequenceID max_sequence)
        {
            if (empty())
            {
                push_back(p);
        ...
        void VerifySorted(PacketSequenceID max_sequence)
        {
