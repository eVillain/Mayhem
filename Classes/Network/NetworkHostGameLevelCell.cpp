...
NetworkHostGameLevelCell::NetworkHostGameLevelCell()
: m_gameLevel(nullptr)
, m_bg(nullptr)
, m_border(nullptr)
...
bool NetworkHostGameLevelCell::init()
{
    ...
    m_gameLevel = HUDHelper::createLabel3x6("Mode", GameViewConstants::FONT_SIZE_MEDIUM);
    m_gameLevel->enableOutline(cocos2d::Color4B::BLACK, 1);
    addChild(m_gameLevel, 1);
...
void NetworkHostGameLevelCell::refreshPositions()
{
    const cocos2d::Vec2 center = GameViewConstants::GAMEMODE_CELL_SIZE * 0.5f;
    const float PADDING = 8.f;
    const cocos2d::Vec2 GAME_LEVEL_POS = cocos2d::Vec2(PADDING + m_gameLevel->getContentSize().width * 0.5f,
                                                       center.y);
    
    m_gameLevel->setPosition(GAME_LEVEL_POS);
}
